...
impl AuthState {
    ...
    pub fn initialize(ctx: &AppContext, api_key: Option<String>) -> Self {
        ...
        if Self::should_use_oss_debug_user() {
            state.set_user(Some(User::test()));
            state.set_credentials(Some(Credentials::ApiKey {
                key: format!("{API_KEY_PREFIX}oss-debug-local-provider"),
                owner_type: None,
            }));
            return state;
        }
    ...
    fn should_use_oss_debug_user() -> bool {
        ChannelState::data_domain() == "dev.warp.WarpOss"
    }
