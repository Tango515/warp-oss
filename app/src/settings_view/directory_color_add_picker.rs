...
impl DirectoryColorAddPicker {
    pub(super) fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(settings_display_text(BUTTON_LABEL), SecondaryTheme)
        ...
        let dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_menu_header_to_static(Box::leak(
                settings_display_text(BUTTON_LABEL).into_boxed_str(),
            ));
            dropdown
        });
        ...
        let add_directory_label = settings_display_text_static(ADD_DIRECTORY_LABEL);
