...
use crate::{
    ...
    settings_view::settings_page::{
        render_body_item, render_dropdown_item, settings_display_text_static, AdditionalInfo,
...
impl ExternalEditorView {
    ...
    fn init_layout_dropdown(
        ...
        let default_option_text = settings_display_text_static("Split Pane");
        let default_app = DropdownItem::new(
            default_option_text.to_string(),
            ExternalEditorAction::SetLayout(EditorLayout::SplitPane),
        );

        let mut items = vec![default_app];
        items.push(DropdownItem::new(
            settings_display_text_static("New Tab"),
        ...
        match layout_to_open_files {
            ...
            EditorLayout::NewTab => {
                dropdown.set_selected_by_name(settings_display_text_static("New Tab"), ctx)
            }
        };
    }
    ...
    fn init_editor_dropdown(
        ...
        let default_option_text = settings_display_text_static("Default App");
        let default_app = DropdownItem::new(
            default_option_text.to_string(),
...
impl View for ExternalEditorView {
    ...
    fn render(&self, app: &warpui::AppContext) -> Box<dyn warpui::Element> {
        ...
        let default_editor = render_dropdown_item(
            appearance,
            settings_display_text_static("Choose an editor to open file links"),
            None,
            None,
        ...
        let code_panels_editor = render_dropdown_item(
            appearance,
            settings_display_text_static("Choose an editor to open files from the code review panel, project explorer, and global search"),
            None,
            None,
            LocalOnlyIconState::for_setting(
        ...
        let default_layout = render_dropdown_item(
            appearance,
            settings_display_text_static("Choose a layout to open files in Warp"),
            None,
            None,
            LocalOnlyIconState::for_setting(
        ...
        if FeatureFlag::TabbedEditorView.is_enabled() {
            column.add_child(render_body_item::<ExternalEditorAction>(
                settings_display_text_static("Group files into single editor pane").to_string(),
                None,
                LocalOnlyIconState::for_setting(
                ...
                appearance
                    ...
                    .finish(),
                Some(settings_display_text_static("When this setting is on, any files opened in the same tab will be automatically grouped into a single editor pane.").to_string()),
            ));
        }
