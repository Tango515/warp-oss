...
use super::settings_page::{
    render_customer_type_badge, settings_display_text, settings_display_text_static, MatchData,
    PageType, SettingsWidget,
};
...
impl std::fmt::Display for TeamsInviteOption {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            "{}",
            match self {
                TeamsInviteOption::Link => settings_display_text_static("Link"),
                TeamsInviteOption::Email => settings_display_text_static("Email"),
            },
        )
    }
}
...
impl TeamsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<TeamsPageView>) -> Self {
        ...
        let team_name_placeholder = tr(current_locale(), MessageId::TeamsTeamName);
        let create_team_editor = Self::editor(
            |me, event, ctx| me.handle_editor_event(event, ctx),
            team_name_placeholder.as_ref(),
            font_size,
            ctx,
        );
        ...
        let approve_domains_block_editor = ctx.add_typed_action_view(|ctx| {
            WordBlockEditorView::new(
                ctx,
                settings_display_text_static("Domains, comma separated"),
                font_size,
                vec![',', ' '],
                MAX_CHIP_WIDTH,
        ...
        let email_invites_block_editor = ctx.add_typed_action_view(|ctx| {
            WordBlockEditorView::new(
                ctx,
                settings_display_text_static("Emails, comma separated"),
                font_size,
                vec![',', ' '],
                MAX_CHIP_WIDTH,
        ...
        let rename_team_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(&settings_display_text("Your new team name"), ctx);
            input
        });
        ...
        let transfer_ownership_modal = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(settings_display_text("Transfer team ownership?")),
    ...
    fn team_to_item_list(&self, team: &Team, current_user_email: &str) -> Vec<Item> {
        ...
        team.pending_email_invites.iter().for_each(|email_invite| {
            ...
            let actions = if current_user_has_admin_permissions {
                vec![ItemAction {
                    icon: Icon::X,
                    label: settings_display_text("Cancel invite"),
            ...
            if member.email != *current_user_email {
                ...
                if current_user_has_owner_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::Users,
                        label: settings_display_text("Transfer ownership"),
                    ...
                    && !team_member_has_owner_permissions
                {
                    if team_member_has_admin_permissions {
                        actions.push(ItemAction {
                            icon: Icon::ArrowDown,
                            label: settings_display_text("Demote from admin"),
                        ...
                        actions.push(ItemAction {
                            icon: Icon::ArrowUp,
                            label: settings_display_text("Promote to admin"),
                            action: TeamsPageAction::SetTeamMemberRole {
                                team_uid: team.uid,
                                user_uid: member.uid,
                ...
                if current_user_has_admin_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::X,
                        label: settings_display_text("Remove from team"),
...
impl TeamsWidget {
        ...
        appearance
            ...
            .with_text_and_icon_label(
                TextAndIcon::new(
                    ...
                    settings_display_text_static("Contact support"),
    ...
    fn render_manage_billing_button(
        ...
        appearance
            ...
            .with_text_and_icon_label(
                TextAndIcon::new(
                    TextAndIconAlignment::IconFirst,
                    settings_display_text_static("Manage billing"),
    ...
    fn render_admin_panel_button(
        ...
        appearance
            ...
            .with_text_and_icon_label(
                TextAndIcon::new(
                    TextAndIconAlignment::IconFirst,
                    settings_display_text_static("Open admin panel"),
        ...
        if has_admin_permissions {
            section.add_child(
                Container::new(self.render_sub_text(
                    tr(current_locale(), MessageId::TeamsInviteLinkInstructions).into_owned(),
                    appearance,
                ...
                section.add_child(
                    Align::new(
                        appearance
                            .ui_builder()
                            .link(
                                settings_display_text_static("Reset links").into(),
                                None,
                    ...
                    if team.billing_metadata.is_on_stripe_paid_plan() {
                        ...
                        manage_billing_link_line.add_child(
                            appearance
                                .ui_builder()
                                .link(
                                    settings_display_text_static("update your payment information")
                                        .into(),
                                    None,
                                    Some(Box::new(move |ctx| {
        ...
        let domains_as_items: Vec<Item> = team
            ...
            .map(|domain_restriction| {
                let actions = if has_admin_permissions {
                    vec![ItemAction {
                        icon: Icon::X,
                        label: settings_display_text("Remove domain"),
    ...
    fn render_leave_or_delete_team_button(
        ...
        let (label, action) = if is_team_owner {
            (
                tr(current_locale(), MessageId::TeamsDeleteTeam),
                TeamsPageAction::ShowDeleteTeamConfirmationDialog,
            )
        } else {
            (
                tr(current_locale(), MessageId::TeamsLeaveTeam),
                TeamsPageAction::ShowLeaveTeamConfirmationDialog,
            )
        };
    ...
    fn render_delete_disabled_help_text(
        ...
        if delete_disabled_reason == TeamDeleteDisabledReason::ActivePaidSubscription {
            let link = appearance
                .ui_builder()
                .link(
                    settings_display_text_static("Manage plan").into(),
                    None,
                    Some(Box::new(move |ctx| {
                        ctx.dispatch_typed_action(
                            TeamsPageAction::GenerateStripeBillingPortalLink { team_uid },
                ...
                let mut row = Flex::row()
                    ...
                    .with_child(
                        Shrinkable::new(
                            1.,
                            Text::new_inline(
                                settings_display_text(item.text.clone()),
    ...
    fn render_copy_link_row(
        ...
        let (link_text, button_enabled) = match &team_metadata.invite_code {
            ...
            None => (
                settings_display_text_static("Failed to load invite link.").into(),
                false,
            ),
        };
        let theme = appearance.theme();
    ...
    fn render_button(
        ...
        let label = settings_display_text(label);
    ...
    fn render_create_team_button(
        ...
        let mut button = appearance
            ...
            .with_centered_text_label(tr(current_locale(), MessageId::TeamsCreate).into_owned())
            .with_style(UiComponentStyles {
                font_color: Some(
                    appearance
                        .theme()
    ...
    fn render_join_team_button(
        ...
        if team_state.team.team_accepting_invites {
            ...
            appearance
                ...
                .with_centered_text_label(settings_display_text("Contact Admin to request access"))
                .disabled()
                .build()
                .finish()
        }
...
impl SettingsWidget for TeamsWidget {
    ...
    fn render(
        ...
        let content = if NetworkStatus::as_ref(app).is_online() {
            ...
            appearance
                .ui_builder()
                .span(tr(current_locale(), MessageId::TeamsOffline).into_owned())
                .build()
                .finish()
        };
