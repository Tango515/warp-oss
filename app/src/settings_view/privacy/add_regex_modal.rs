...
impl AddRegexModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let name_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(&settings_display_text("e.g. \"Google API Key\""), ctx);
            editor
        });
...
impl View for AddRegexModal {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let name_label = Text::new(
            settings_display_text("Name (optional)"),
        ...
        let regex_label = Text::new(
            settings_display_text("Regex pattern"),
            appearance.ui_font_family(),
            LABEL_FONT_SIZE,
        )
        .with_color(theme.active_ui_text_color().into())
        ...
        let mut add_button = appearance
            ...
            .with_text_label(settings_display_text("Add regex"))
            ...
            .with_child(
                Expanded::new(
                    ...
                    Container::new(if !is_valid_regex && !pattern_text.trim().is_empty() {
                        Text::new(
                            settings_display_text("Invalid regex"),
                            appearance.ui_font_family(),
                            LABEL_FONT_SIZE,
                        )
                        .with_color(
