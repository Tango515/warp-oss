...
use super::{
    settings_page::{
        settings_display_text, MatchData, PageType, SettingsPageMeta, SettingsPageViewHandle,
        SettingsWidget, PAGE_PADDING,
    },
    SettingsSection,
};
use crate::{
...
impl ReferralsPageView {
    pub fn new(referrals_client: Arc<dyn ReferralsClient>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let page = PageType::new_monolith(
            ...
            Some(tr(current_locale(), MessageId::ReferralsHeader).into_owned()),
            true,
        );
        Self {
            page,
        ...
        match &self.api_state {
            ...
            ApiState::Ready { referral_info, .. } => {
                ...
                ctx.emit(ReferralsPageEvent::ShowToast {
                    message: tr(current_locale(), MessageId::ReferralsLinkCopied).into_owned(),
    ...
    fn handle_send_email_invite_response(
        ...
        match response {
            Ok(successful) => {
                ...
                ctx.emit(ReferralsPageEvent::ShowToast {
                    message: tr(current_locale(), MessageId::ReferralsEmailSuccess).into_owned(),
            ...
            Err(err) => {
                ...
                ctx.emit(ReferralsPageEvent::ShowToast {
                    message: tr(current_locale(), MessageId::ReferralsEmailFailure).into_owned(),
                    flavor: ToastFlavor::Error,
                });
            }
        }
...
impl ReferralsWidget {
    ...
    fn render_link_row(
        ...
        let (link_text, button_enabled) = match &view.api_state {
            ...
            ApiState::Loading => (
                tr(current_locale(), MessageId::ReferralsLoading).into_owned(),
                false,
            ),
            ApiState::Failed => (
                tr(current_locale(), MessageId::ReferralsFailedLoad).into_owned(),
                false,
            ),
        };
        ...
        Container::new(
            Flex::row()
                ...
                .with_child(self.render_button(
                    button_enabled,
                    &tr(current_locale(), MessageId::ReferralsCopyLink).into_owned(),
    ...
    fn render_email_row(
        ...
        let (button_text, button_enabled) = match &view.api_state {
            ApiState::Ready {
                ...
                tr(current_locale(), MessageId::ReferralsSend).into_owned(),
                true,
            ),
            ApiState::Ready {
                ...
                tr(current_locale(), MessageId::ReferralsSending).into_owned(),
                false,
            ),
            _ => (
                tr(current_locale(), MessageId::ReferralsSend).into_owned(),
            ...
            .with_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .span(
                            tr(current_locale(), MessageId::ReferralsAnonymousHeader).into_owned(),
                        )
                        .with_style(UiComponentStyles {
    ...
    fn render_rewards_section(
        ...
        rewards_section.add_child(
            Container::new(
                appearance
                    .ui_builder()
                    .span(tr(current_locale(), MessageId::ReferralsRewardIntro).into_owned())
                    .with_style(UiComponentStyles {
                    ...
                    FormattedTextElement::new(
                        FormattedText::new([FormattedTextLine::Line(vec![
                            ...
                            FormattedTextFragment::hyperlink(
                                tr(current_locale(), MessageId::ReferralsCertainRestrictions)
                                    .into_owned(),
                                TERMS_URL,
                            ),
        ...
        Flex::row()
            ...
            .with_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .span(settings_display_text(reward.label.clone()))
                        .with_style(UiComponentStyles {
    ...
    fn render_claimed_referrals_count(
        ...
        let current_referrals_label = match claimed_count {
            1 => tr(current_locale(), MessageId::ReferralsCurrentReferral).into_owned(),
            _ => tr(current_locale(), MessageId::ReferralsCurrentReferrals).into_owned(),
        };
