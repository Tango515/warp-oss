use super::{
    settings_page::{
        render_page_title, settings_display_text, MatchData, PageType, SettingsPageMeta,
        SettingsPageViewHandle, SettingsWidget, HEADER_FONT_SIZE, PAGE_PADDING,
    },
    SettingsSection,
};
...
impl UserOwnedBlock {
    ...
    fn copy_link_button(&self, appearance: &Appearance, block_url: String) -> Box<dyn Element> {
        let button = appearance
            ...
            .with_text_label(settings_display_text("Copy link"));
    ...
    fn link_text(&self, appearance: &Appearance, block_url: String) -> Box<dyn Element> {
        if self.unshare_request_status == UnshareBlockRequestState::InFlight {
            appearance
                .ui_builder()
                .label(settings_display_text("Deleting..."))
                .with_style(
            ...
            Container::new(
                Flex::column()
                    .with_child(
                        Align::new(
                            ui_builder
                                .label(settings_display_text("Unshare block"))
                                .with_style(UiComponentStyles {
                                ...
                                Flex::row()
                                    .with_child(
                                        ui_builder
                                            ...
                                            .with_text_label(
                                                tr(current_locale(), MessageId::KeybindingsCancel)
                                                    .into_owned(),
                                            )
                                            .build()
                                            .on_click(|ctx, _, _| {
                                    ...
                                    .with_child(
                                        Container::new(
                                            ui_builder
                                                ...
                                                .with_text_label(settings_display_text("Unshare"))
                                                .build()
                                                .on_click(|ctx, _, _| {
                                                    ctx.dispatch_typed_action(
