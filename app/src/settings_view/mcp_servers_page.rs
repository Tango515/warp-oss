...
use crate::{
    ...
    settings_view::{
        ...
        settings_page::{
            settings_display_text, MatchData, PageType, SettingsPageMeta, SettingsWidget,
        },
        SettingsSection,
    },
...
impl MCPServersSettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        Self {
            page: PageType::new_monolith(
                ...
                Some(settings_display_text(PAGE_TITLE_TEXT)),
                true,
            ),
    ...
    fn handle_log_out(
        ...
        let message = match server_name {
            Some(name) => format!(
                "{} {name} {}",
                settings_display_text("Successfully logged out of"),
                settings_display_text("MCP server")
            ),
            None => settings_display_text("Successfully logged out of MCP server"),
        };
        match item_id {
    ...
    pub fn autoinstall_from_gallery(
        ...
        let Some(gallery_server) = gallery_server else {
            ...
            self.add_error_toast(
                format!(
                    "{} '{autoinstall_param}'",
                    settings_display_text("Unknown MCP server")
                ),
                ctx,
            );
            return;
        ...
        let Ok(templatable_mcp_server) = TemplatableMCPServer::try_from(gallery_server) else {
            ...
            self.add_error_toast(
                format!(
                    "{} '{gallery_title}' {}",
                    settings_display_text("MCP server"),
                    settings_display_text("cannot be installed from this link.")
                ),
                ctx,
            );
            return;
