use super::{
    ...
    editor_text_colors,
    settings_page::{
        settings_display_text, settings_display_text_static, MatchData, PageType,
...
fn page_title() -> &'static str {
    settings_display_text_static(PAGE_TITLE_TEXT)
}
fn page_description() -> &'static str {
    settings_display_text_static(PAGE_DESCRIPTION_TEXT)
}
...
impl EnvironmentDisplayData {
    ...
    fn format_timestamp_text(&self) -> String {
        ...
        let last_used_part = match self.last_used_ts {
            Some(ts) => format!(
                "{}{}",
                settings_display_text("Last used: "),
                format_approx_duration_from_now_utc(ts.utc())
            ),
            None => settings_display_text("Last used: never"),
        };
...
impl EnvironmentsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let search_editor = Self::create_single_line_editor(
            Box::leak(settings_display_text("Search environments...").into_boxed_str()),
            ctx,
        );
    ...
    fn handle_update_manager_event(
            ...
            (&result.operation, &result.success_type)
        {
            ...
            if should_handle {
                ...
                self.show_success_toast(
                    settings_display_text("Successfully updated environment"),
                    ctx,
                );
            ...
            if let Some(pending_client_id) = self.pending_create_client_id.take() {
                ...
                if let Some(result_client_id) = &result.client_id {
                    if *result_client_id == pending_client_id {
                        self.show_success_toast(
                            settings_display_text("Successfully created environment"),
                            ctx,
                        );
                    }
                }
            ...
            if let Some(pending_env_id) = self.pending_delete_env_id.take() {
                ...
                if let Some(server_id) = &result.server_id {
                    if server_id.uid() == pending_env_id.uid() {
                        self.show_success_toast(
                            settings_display_text("Environment deleted successfully"),
                            ctx,
                        );
                    }
                }
        ...
        if matches!(&result.operation, ObjectOperation::MoveToDrive) {
            ...
            if matches!(result.success_type, OperationSuccessType::Success) {
                self.show_success_toast(
                    settings_display_text("Successfully shared environment"),
                    ctx,
                );
            } else {
                self.show_error_toast(
                    settings_display_text("Failed to share environment with team"),
                    ctx,
                );
            }
        ...
        match event {
            UpdateEnvironmentFormEvent::Created {
                ...
                let Some(owner) = owner else {
                    self.show_error_toast(
                        settings_display_text("Unable to create environment: not logged in."),
                        ctx,
                    );
                    return;
                };
            ...
            UpdateEnvironmentFormEvent::Updated {
                ...
                let Some(existing_env) = CloudAmbientAgentEnvironment::get_by_id(env_id, ctx)
                    ...
                    self.show_error_toast(
                        settings_display_text("Unable to save: environment no longer exists."),
                        ctx,
                    );
                    return;
                };
...
impl EnvironmentsPageWidget {
    fn render_list_page(
        ...
        let description = appearance
            .ui_builder()
            .paragraph(settings_display_text_static(PAGE_DESCRIPTION_TEXT))
    ...
    fn render_no_matches_state(appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            Text::new(
                settings_display_text_static("No environments match your search."),
                appearance.ui_font_family(),
    ...
    fn render_scoped_section(
        ...
        let header = match list_scope {
            EnvironmentListScope::Personal => {
                Self::render_overline_header(settings_display_text_static("Personal"), appearance)
            }
            EnvironmentListScope::Team => {
                let shared_by_text = UserWorkspaces::as_ref(app)
                    ...
                    .map(|team| {
                        format!(
                            "{}{}",
                            settings_display_text("Shared by Warp and "),
                            team.name
                        )
                    })
                    .unwrap_or_else(|| settings_display_text("Shared by Warp and your team"));
    ...
    fn render_empty_state(
        ...
        let launch_agent_label = settings_display_text("Launch agent");
        ...
        let github_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                icon: Icon::Github,
                title: settings_display_text("Quick setup"),
                badge: Some(settings_display_text("Suggested")),
                subtitle: settings_display_text(
        ...
        let local_repos_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                icon: Icon::Terminal,
                title: settings_display_text("Use the agent"),
                badge: None,
                subtitle: settings_display_text(
        ...
        let header = Text::new(
            settings_display_text_static("You haven’t set up any environments yet."),
            appearance.ui_font_family(),
        ...
        let subheader = Text::new(
            settings_display_text_static("Choose how you’d like to set up your environment:"),
            appearance.ui_font_family(),
    ...
    fn render_empty_state_row(
        ...
        let badge: Option<&'static str> = badge.map(|b| &*Box::leak(b.into_boxed_str()));
        let subtitle: &'static str = Box::leak(subtitle.into_boxed_str());
        let theme = appearance.theme();
        let build_icon = || {
            Container::new(
    ...
    fn render_environment_card(
        ...
        Hoverable::new(card_hover_mouse_state, move |state| {
            ...
            let view_runs_link = appearance
                .ui_builder()
                .link(
                    settings_display_text("View my runs"),
                    None,
