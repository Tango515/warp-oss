use super::{
    editor_text_colors,
    settings_page::{render_input_list, settings_display_text, InputListItem},
};
    ...
    fn new_impl(
        ...
        let setup_commands_input = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(
                &settings_display_text("e.g. cd my-repo && pip install -r requirements.txt"),
                ctx,
            );
        ...
        if self.should_show_share_with_team_checkbox(app) {
            ...
            let label = Hoverable::new(
                ...
                move |state| {
                    ...
                    Text::new_inline(
                        settings_display_text("Share with team"),
                        font_family,
                        font_size,
                    )
    ...
    fn render_image_link_button(&self, appearance: &Appearance) -> Option<Box<dyn Element>> {
        ...
        let button = Container::new(
            ...
            .with_tooltip({
                ...
                move || {
                    ui_builder
                        .tool_tip(settings_display_text(format!(
                            "Open image at {docker_hub_url}"
                        )))
                        .build()
                        .finish()
                }
