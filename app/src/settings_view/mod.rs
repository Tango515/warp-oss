...
use crate::i18n::{tr, Locale, MessageId};
...
use language_page::{LanguagePageView, LanguagePageViewEvent};
...
use settings_page::{
    settings_display_text, MatchData, SettingsPage, SettingsPageEvent, SettingsPageMeta,
    SettingsPageViewHandle, HEADER_PADDING,
};
...
impl Display for SettingsSection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.localized_title(Locale::EnUs))
    }
}

impl SettingsSection {
    fn title_message_id(&self) -> MessageId {
        match self {
            Self::About => MessageId::SettingsAbout,
            Self::Account => MessageId::SettingsAccount,
            Self::AI => MessageId::SettingsWarpAgent,
            Self::MCPServers => MessageId::SettingsMcpServers,
            Self::BillingAndUsage => MessageId::SettingsBillingAndUsage,
            Self::Appearance => MessageId::SettingsAppearance,
            Self::Code => MessageId::SettingsCode,
            Self::LocalAIProvider => MessageId::SettingsLocalAIProvider,
            Self::Features => MessageId::SettingsFeatures,
            Self::Keybindings => MessageId::SettingsKeybindings,
            Self::Language => MessageId::SettingsLanguage,
            Self::Privacy => MessageId::SettingsPrivacy,
            Self::Referrals => MessageId::SettingsReferrals,
            Self::SharedBlocks => MessageId::SettingsSharedBlocks,
            Self::Teams => MessageId::SettingsTeams,
            Self::WarpDrive => MessageId::SettingsWarpDrive,
            Self::Warpify => MessageId::SettingsWarpify,
            Self::WarpAgent => MessageId::SettingsWarpAgent,
            Self::AgentProfiles => MessageId::SettingsProfiles,
            Self::AgentMCPServers => MessageId::SettingsMcpServers,
            Self::Knowledge => MessageId::SettingsKnowledge,
            Self::ThirdPartyCLIAgents => MessageId::SettingsThirdPartyCliAgents,
            Self::CodeIndexing => MessageId::SettingsCodeIndexing,
            Self::EditorAndCodeReview => MessageId::SettingsCodeEditorAndCodeReview,
            Self::CloudEnvironments => MessageId::SettingsCloudEnvironments,
            Self::OzCloudAPIKeys => MessageId::SettingsOzCloudApiKeys,
        }
    }

    pub fn localized_title(&self, locale: Locale) -> String {
        if locale == Locale::EnUs && matches!(self, Self::AgentMCPServers) {
            return "MCP servers".to_string();
        }

        tr(locale, self.title_message_id()).into_owned()
    }
    ...
    pub fn parent_page_section(&self) -> Self {
        match self {
            ...
            // LocalAIProvider is grouped under Agents but renders its own page.
            Self::LocalAIProvider => Self::LocalAIProvider,
...
impl FromStr for SettingsSection {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            "Code" => Ok(Self::Code),
            "LocalAIProvider" | "Local AI Provider" => Ok(Self::LocalAIProvider),
...
macro_rules! update_page {
    ($handle:expr, $update:expr, $ctx:expr) => {
        match $handle {
            ...
            SettingsPageViewHandle::LocalAIProvider(handle) => $ctx.update_view(handle, $update),
...
impl SettingsView {
    pub fn new(page: Option<SettingsSection>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let language_page_handle = ctx.add_typed_action_view(LanguagePageView::new);
        ctx.subscribe_to_view(&language_page_handle, |_, _, event, ctx| match event {
            LanguagePageViewEvent::LocaleChanged => ctx.notify(),
        });

        // Local AI Provider page
        let local_ai_provider_page_handle =
            ctx.add_typed_action_view(local_ai_provider_page::LocalAIProviderPageView::new);
        ...
        let search_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(&settings_display_text("Search"), ctx);
            editor
        });
        ...
        let mut settings_pages = vec![
            ...
            SettingsPage::new(local_ai_provider_page_handle),
        ...
        let mut nav_items = vec![
            ...
            SettingsNavItem::Umbrella(SettingsUmbrella::new(
                MessageId::SettingsAgents,
                SettingsSection::ai_subpages().to_vec(),
            )),
            SettingsNavItem::Page(SettingsSection::BillingAndUsage),
            SettingsNavItem::Umbrella(SettingsUmbrella::new(
                MessageId::SettingsCode,
                vec![
            ...
            SettingsNavItem::Umbrella(SettingsUmbrella::new(
                MessageId::SettingsCloudPlatform,
                vec![
            ...
            EditorEvent::Edited(_) => {
                ...
                if is_search_active {
                    ...
                    if current.is_ai_subpage()
                        ...
                        && current != SettingsSection::LocalAIProvider
                    {
    ...
    pub fn set_and_refresh_current_page_internal(
        ...
        if section.is_subpage() {
            ...
            if section.is_ai_subpage()
                ...
                && section != SettingsSection::LocalAIProvider
            {
    ...
    fn should_render_page(&self, settings_page: &SettingsPage, app: &AppContext) -> bool {
        match &settings_page.view_handle {
            ...
            SettingsPageViewHandle::LocalAIProvider(v) => v.as_ref(app).should_render(app),
        ...
        Container::new(
            Align::new(
                ...
                .with_cross_axis_alignment(CrossAxisAlignment::Center)
                    .with_children([
                        Text::new(
                            settings_display_text("No settings match your search."),
