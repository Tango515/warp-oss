...
use super::{
    flags,
    settings_page::{
        build_sub_header, render_body_item, render_separator, settings_display_text,
        settings_display_text_static, Category, MatchData, PageType, SettingsPageMeta,
        SettingsPageViewHandle, SettingsWidget, HEADER_PADDING, TOGGLE_BUTTON_RIGHT_PADDING,
    },
...
use crate::{
    ...
    i18n::{current_locale, tr, MessageId},
...
fn code_feature_name() -> Cow<'static, str> {
    tr(current_locale(), MessageId::SettingsCode)
}

fn initialization_settings_header() -> Cow<'static, str> {
    tr(current_locale(), MessageId::CodeInitializationSettings)
}

fn codebase_indexing_label() -> Cow<'static, str> {
    tr(current_locale(), MessageId::CodeCodebaseIndexing)
}

fn codebase_index_description() -> Cow<'static, str> {
    tr(current_locale(), MessageId::CodeCodebaseIndexDescription)
}

fn warp_indexing_ignore_description() -> Cow<'static, str> {
    tr(
        current_locale(),
        MessageId::CodeWarpIndexingIgnoreDescription,
    )
}

fn auto_index_feature_name() -> Cow<'static, str> {
    tr(current_locale(), MessageId::CodeAutoIndexFeatureName)
}

fn auto_index_description() -> Cow<'static, str> {
    tr(current_locale(), MessageId::CodeAutoIndexDescription)
}

fn indexing_disabled_admin_text() -> Cow<'static, str> {
    tr(current_locale(), MessageId::CodeIndexingDisabledAdmin)
}

fn indexing_workspace_enabled_admin_text() -> Cow<'static, str> {
    tr(
        current_locale(),
        MessageId::CodeIndexingWorkspaceEnabledAdmin,
    )
}

fn indexing_disabled_global_ai_text() -> Cow<'static, str> {
    tr(current_locale(), MessageId::CodeIndexingDisabledGlobalAi)
}

fn codebase_index_limit_reached() -> Cow<'static, str> {
    tr(current_locale(), MessageId::CodeCodebaseIndexLimitReached)
}
...
impl CodeSubpage {
    ...
    pub fn title(&self) -> Cow<'static, str> {
        match self {
            Self::Indexing => tr(current_locale(), MessageId::CodeCodebaseIndexingCategory),
            Self::EditorAndCodeReview => {
                tr(current_locale(), MessageId::CodeEditorAndReviewCategory)
            }
        }
    }
}
...
impl CodeSettingsPageView {
    pub fn new(ctx: &mut ViewContext<CodeSettingsPageView>) -> Self {
        ...
        let manual_add_directory_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(
                settings_display_text_static("Index new folder"),
                SecondaryTheme,
            )
        ...
        let page = if FeatureFlag::OpenWarpNewSettingsModes.is_enabled() {
            ...
            let categories = vec![
                Category::new(
                    settings_display_text_static("Codebase Indexing"),
                    codebase_indexing_widgets,
                ),
                Category::new(
                    settings_display_text_static("Code Editor and Review"),
...
impl CodePageWidget {
        ...
        content.add_child(
            Container::new(
                Flex::row()
                    ...
                    .with_child(
                        ui_builder
                            .span(settings_display_text("Initialized / indexed folders"))
                            .with_style(UiComponentStyles {
                                font_size: Some(16.0),
                                font_weight: Some(Weight::Semibold),
                                font_color: Some(theme.active_ui_text_color().into()),
        ...
        if workspaces.is_empty() {
            content.add_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .paragraph(settings_display_text(
            ...
            let open_rules_button = ui_builder
                ...
                .with_text_and_icon_label(
                    warpui::ui_components::button::TextAndIcon::new(
                        ...
                        settings_display_text_static("Open project rules"),
    ...
    fn render_indexing_subsection(
        ...
        column.add_child(
            ui_builder
                .span(settings_display_text("INDEXING"))
                .with_style(UiComponentStyles {
                    font_size: Some(11.0),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(theme.disabled_ui_text_color().into()),
        ...
        if let Some(index_status) = index_status {
            ...
            column.add_child(
                Flex::row()
                    ...
                    .with_child(
                        ui_builder
                            .label(settings_display_text_static("No index created"))
                            .with_style(UiComponentStyles {
        ...
        let (status_text, status_color) = if index_state.has_pending() {
            let progress_text = match index_state.sync_progress() {
                Some(SyncProgress::Discovering { total_nodes }) => Cow::from(
                    settings_display_text(format!("Discovered {total_nodes} chunks")),
                ),
                Some(SyncProgress::Syncing {
                    completed_nodes,
                    total_nodes,
                }) => Cow::from(settings_display_text(format!(
                    "Syncing - {completed_nodes} / {total_nodes}"
                ))),
                None => Cow::from(settings_display_text_static("Syncing...")),
            };
            ...
            let (text, color, status_icon) = if completed_successfully {
                (
                    settings_display_text_static("Synced"),
                    theme.ansi_fg_green(),
                    Icon::Check,
                )
            ...
            {
                (
                    settings_display_text_static("Codebase too large"),
                    theme.ui_warning_color(),
                    Icon::AlertTriangle,
                )
            } else if index_state.has_synced_version() {
                (
                    settings_display_text_static("Stale"),
                    theme.nonactive_ui_detail().into_solid(),
                    Icon::ClockRefresh,
                )
            } else {
                (
                    settings_display_text_static("Failed"),
            ...
            (
                Cow::from(settings_display_text_static("No index built")),
    ...
    fn render_lsp_servers_subsection(
        ...
        content.add_child(
            ui_builder
                .span(settings_display_text("LSP SERVERS"))
                .with_style(UiComponentStyles {
                    font_size: Some(11.0),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(theme.disabled_ui_text_color().into()),
    ...
    fn render_suggested_lsp_server_row(
        ...
        let (description, is_installing) = match &repo_status {
            Some(LspRepoStatus::DisabledAndInstalled { .. }) => {
                (settings_display_text_static("Installed"), false)
            }
            Some(LspRepoStatus::Installing { .. }) => {
                (settings_display_text_static("Installing..."), true)
            }
            Some(LspRepoStatus::CheckingForInstallation) => {
                (settings_display_text_static("Checking..."), true)
            }
            _ => (
                settings_display_text_static("Available for download"),
                false,
            ),
        };
        ...
        if is_failed {
            if let Some(server_handle) = server_model.cloned() {
                ...
                let restart_button = ui_builder
                    ...
                    .with_text_label(settings_display_text("Restart server"))
                    .build()
                    .with_cursor(Cursor::PointingHand)
                    .on_click(move |ctx, _, _| {
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            if has_logs {
                ...
                let view_logs_button = ui_builder
                    ...
                    .with_text_label(settings_display_text("View logs"))
                    .build()
                    .with_cursor(Cursor::PointingHand)
                    .on_click(move |ctx, _, _| {
                ...
                match server.state() {
                    LspState::Available { .. } if !server.has_pending_tasks() => (
                        AnsiColorIdentifier::Green
                            ...
                            .into(),
                        settings_display_text_static("Available"),
                    ),
                    LspState::Starting | LspState::Available { .. } => (
                        AnsiColorIdentifier::Yellow
                            .to_ansi_color(&theme.terminal_colors().normal)
                            .into(),
                        settings_display_text_static("Busy"),
                    ),
                    LspState::Failed { .. } => (
                    ...
                    LspState::Stopped { .. } | LspState::Stopping { .. } => (
                        ...
                        settings_display_text_static("Stopped"),
                    ),
                }
            }
            None => (
                theme.disabled_ui_text_color().into_solid(),
                settings_display_text_static("Not running"),
            ),
        }
    }
}
...
impl SettingsWidget for AutoOpenCodeReviewPaneCodeWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            settings_display_text_static("Auto open code review panel").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(settings_display_text("When this setting is on, the code review panel will open on the first accepted diff of a conversation")),
        )
    }
}
...
impl SettingsWidget for CodeReviewDiffStatsToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            settings_display_text_static("Show diff stats on code review button").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
...
impl SettingsWidget for ProjectExplorerToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            settings_display_text_static("Project explorer").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
...
impl SettingsWidget for GlobalSearchToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            settings_display_text_static("Global file search").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
