...
impl PlatformPageView {
    ...
    pub fn new(ctx: &mut ViewContext<PlatformPageView>) -> Self {
        ...
        let create_api_key_modal_view = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(settings_display_text("New API key")),
                create_api_key_body,
                ctx,
            )
    ...
    fn show_create_api_key_modal(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        self.create_api_key_modal_state
            .set_title(Some(settings_display_text("New API key")), ctx);
            ...
            ExpireApiKeyButtonEvent::ExpireApiKeySucceeded { uid } => {
                ...
                crate::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = crate::view_components::DismissibleToast::success(
                        settings_display_text("API key deleted").to_string(),
                    );
                    toast_stack.add_ephemeral_toast(toast, window_id, ctx);
                });
                ctx.notify();
...
impl PlatformPageWidget {
    fn render_description_with_link(
        ...
        let text = vec![
            FormattedTextFragment::plain_text(settings_display_text("Create and manage API keys to allow other Oz cloud agents to access your Warp account.\nFor more information, visit the ")),
            FormattedTextFragment::hyperlink(settings_display_text("Documentation."), API_KEY_DOCS_URL),
        ];
    ...
    fn render_api_keys_section(
        ...
        col.add_child(
            Flex::row()
                ...
                .with_child(
                    Text::new_inline(
                        settings_display_text("Oz Cloud API Keys"),
                        appearance.ui_font_family(),
                        16.,
                    )
                ...
                .with_child(
                    ui_builder
                        ...
                        .with_text_label(settings_display_text("+ Create API Key"))
                        .build()
                        .on_click(|ctx, _, _| {
    ...
    fn render_api_key_row(
        ...
        let last_used = key
            ...
            .unwrap_or_else(|| settings_display_text("Never").to_owned());
        let expires_at = key
            .expires_at
            ...
            Align::new(
                Flex::column()
                    ...
                    .with_child(
                        Container::new(
                            Text::new(
                                settings_display_text("No API Keys"),
                                appearance.ui_font_family(),
                                SUBHEADER_FONT_SIZE,
                            )
                            .with_color(appearance.theme().active_ui_text_color().into())
