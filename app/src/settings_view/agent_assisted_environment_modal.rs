...
use crate::{
    ...
    settings_view::settings_page::{settings_display_text, settings_display_text_static},
    themes::theme::Blend,
    ui_components::{
        buttons::icon_button,
...
impl AgentAssistedEnvironmentModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let add_repo_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(settings_display_text("Add repo"), SecondaryTheme)
        ...
        let create_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(settings_display_text("Create environment"), PrimaryTheme).on_click(
                |ctx| {
    ...
    fn render_selected_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        col.add_child(
            self.render_section_title(&settings_display_text("Selected repos"), appearance),
        );

        if self.selected_repo_paths.is_empty() {
            col.add_child(
                Text::new(
                    settings_display_text_static("No repos selected yet"),
                    appearance.ui_font_family(),
        ...
        for (idx, repo_path) in self.selected_repo_paths.iter().enumerate() {
            ...
            let name = settings_display_text(
                repo_path
                    .file_name()
                    .and_then(|s| s.to_str())
                    .unwrap_or("(unknown)"),
    ...
    fn render_available_section(&self, appearance: &Appearance) -> Box<dyn Element> {
            ...
            .with_child(
                Expanded::new(
                    ...
                    self.render_section_title(
                        &settings_display_text("Available indexed repos"),
                        appearance,
                    ),
                )
                .finish(),
        ...
        if self.available_repos.is_empty() {
            let text = settings_display_text(
                if cfg!(all(feature = "local_fs", not(target_family = "wasm"))) {
        ...
        if !has_any_available {
            col.add_child(
                Text::new(
                    settings_display_text_static("All locally indexed repos are already selected."),
                    appearance.ui_font_family(),
                    appearance.ui_font_size() * 0.95,
                )
                .with_color(theme.nonactive_ui_text_color().into())
    ...
    fn show_not_a_repo_toast(&self, selected_path: &Path, ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = DismissibleToast::error(format!(
                "{}: {path}",
                settings_display_text("Selected folder is not a Git repository")
            ))
            ...
            move |paths_result, ctx| {
                let result = paths_result.and_then(|paths| {
                    paths.into_iter().next().map(PathBuf::from).ok_or_else(|| {
                        FilePickerError::DialogFailed(settings_display_text(
    ...
    fn render_dialog(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let dialog = Dialog::new(
            settings_display_text("Select repos for your environment"),
            Some(description),
