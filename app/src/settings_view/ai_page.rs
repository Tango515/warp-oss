...
use super::{
    ...
    settings_page::{
        ...
        settings_display_text, settings_display_text_static, InputListItem, LocalOnlyIconState,
...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let code_read_allowlist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(&settings_display_text("e.g. ~/code-repos/repo"), ctx);
            input
        });
        Self::update_editor_interaction_state(
        ...
        let autodetection_denylist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(&settings_display_text("Commands, comma separated"), ctx);
        ...
        let command_execution_allowlist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(&settings_display_text("e.g. ls .*"), ctx);
            input
        });
        Self::update_editor_interaction_state(
        ...
        let command_execution_denylist_editor = ctx.add_typed_action_view(|ctx| {
            let mut input =
                SubmittableTextInput::new(ctx).validate_on_edit(|s| Regex::new(s).is_ok());
            input.set_placeholder_text(&settings_display_text("e.g. rm .*"), ctx);
            input
        });
        Self::update_editor_interaction_state(
        ...
        let cli_agent_footer_command_editor = ctx.add_typed_action_view(|ctx| {
            let mut input =
                SubmittableTextInput::new(ctx).validate_on_edit(|s| Regex::new(s).is_ok());
            input.set_placeholder_text(&settings_display_text("command (supports regex)"), ctx);
            input
        });
        ...
        let write_to_pty_autonomy_dropdown_menu = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_items(
                vec![
                    ...
                    DropdownItem::new(
                        settings_display_text("Ask on first write"),
    ...
    fn refresh_autonomy_dropdown_menu(
        ...
        menu.update(ctx, |menu, ctx| {
            ...
            menu.set_items(
                vec![
                    DropdownItem::new(
                        settings_display_text("Read only"),
                        AISettingsPageAction::SetAutonomyReadonlyCommandsSetting,
                    ),
                    DropdownItem::new(
                        settings_display_text("Supervised"),
...
fn render_ai_setting_description_with_font_size(
    ...
    ui_builder
        .paragraph(settings_display_text(description.into().into_owned()))
...
fn render_toolbar_layout_editor(
    ...
    let label = Container::new(
        appearance
            .ui_builder()
            .span(settings_display_text("Toolbar layout"))
            .with_style(UiComponentStyles {
    ...
    fn render(
        ...
        if is_ai_disabled_due_to_remote_session_org_policy {
            row.add_child(
                ConstrainedBox::new(
                    Container::new(
                        Text::new(settings_display_text("Your organization disallows AI when the active pane contains content from a remote session"), appearance.ui_font_family(), 12.)
                    ...
                    .with_child(
                        Container::new(
                            ui_builder
                                ...
                                .with_text_label(settings_display_text("Sign up"))
                                .build()
                                .on_click(move |ctx, _, _| {
...
impl UsageWidget {
    fn render_request_usage_count(
        ...
        let request_count_label = if workspace_is_delinquent_due_to_payment_issue {
            settings_display_text("Restricted due to billing issue")
        } else if is_unlimited {
            settings_display_text("Unlimited")
        } else {
            format!("{used}/{limit}")
        };
                    ...
                    Container::new(
                        Flex::column()
                            .with_child(
                                appearance
                                    .ui_builder()
                                    .paragraph(settings_display_text(header.into().into_owned()))
                                    .with_style(UiComponentStyles {
...
impl SettingsWidget for UsageWidget {
    ...
    fn render(
            ...
            UserWorkspaces::as_ref(app).current_team()
        {
            ...
            vec![
                FormattedTextFragment::hyperlink("Upgrade", upgrade_url),
                FormattedTextFragment::plain_text(settings_display_text(" to get more AI usage.")),
            ]
        };
    ...
    fn render(
        ...
        if FeatureFlag::ProfilesDesignRevamp.is_enabled() {
            ...
            agents_header.add_child(render_ai_setting_description(
                settings_display_text("Set the boundaries for how your Agent operates. Choose what it can access, how much autonomy it has, and when it must ask for your approval. You can also fine-tune behavior around natural language input, codebase awareness, and more."),
                ai_settings.is_any_ai_enabled(app),
                app,
            ));
...
impl AgentsWidget {
    fn render_profiles_section(
        ...
        let header_and_description = Flex::column()
            .with_child(
                build_sub_header(
                    appearance,
                    settings_display_text("Profiles"),
                    Some(styles::header_font_color(is_any_ai_enabled, app)),
                )
                .finish(),
            )
            .with_child(
                Container::new(
                    render_ai_setting_description(
                        settings_display_text("Profiles let you define how your Agent operates — from the actions it can take and when it needs approval, to the models it uses for tasks like coding and planning. You can also scope them to individual projects."),
                        is_any_ai_enabled,
                        app,
                    )
                )
    ...
    fn render_context_window_setting(
        ...
        let label = Container::new(render_body_item_label::<AISettingsPageAction>(
            settings_display_text("Context window (tokens)"),
            None,
            None,
            ...
            Container::new(
                Flex::row()
                    ...
                    .with_children([
                        ...
                        appearance
                            .ui_builder()
                            .span(settings_display_text("Show model picker in prompt"))
                            .with_style(UiComponentStyles {
                                font_color: Some(
...
impl AIFactWidget {
    fn render_rules_toggle(
        ...
        let rules_description = vec![
            FormattedTextFragment::plain_text(
                settings_display_text("Rules help the Warp Agent follow your conventions, whether for codebases or specific workflows. "),
            ),
            FormattedTextFragment::hyperlink(
                settings_display_text_static("Learn more"),
...
impl SettingsWidget for CLIAgentWidget {
    ...
    fn render(
        ...
        let description_fragments = vec![
            ...
            FormattedTextFragment::plain_text(settings_display_text(", or ")),
        ...
        if is_footer_enabled {
            ...
            let command_list = {
                ...
                list_column.add_child(
                    appearance
                        .ui_builder()
                        .span(settings_display_text("Commands that enable the toolbar"))
                        .with_style(UiComponentStyles {
                            font_size: Some(CONTENT_FONT_SIZE),
                            ..Default::default()
                        })
...
impl AwsBedrockWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        let aws_auth_refresh_command_editor = ctx.add_typed_action_view(move |ctx| {
            ...
            let mut editor = EditorView::single_line(options, ctx);
            editor.set_placeholder_text(&settings_display_text("aws login"), ctx);
        ...
        let aws_auth_refresh_profile_editor = ctx.add_typed_action_view(move |ctx| {
            ...
            let mut editor = EditorView::single_line(options, ctx);
            editor.set_placeholder_text(&settings_display_text("default"), ctx);
