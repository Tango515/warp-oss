...
fn referral_cta() -> String {
    tr(current_locale(), MessageId::SettingsReferralCta).into_owned()
}
const REGULAR_TEXT_FONT_SIZE: f32 = 12.;
const VERTICAL_MARGIN: f32 = 24.;
fn log_out_text() -> String {
    tr(current_locale(), MessageId::SettingsLogOut).into_owned()
}
lazy_static! {
...
impl From<&MainPageAction> for LoginGatedFeature {
    fn from(val: &MainPageAction) -> LoginGatedFeature {
        ...
        match val {
            ...
            ToggleSettingsSync => "Settings Sync",
            _ => "Unknown reason",
        }
    }
}
...
impl MainSettingsPageView {
    pub fn new(ctx: &mut ViewContext<MainSettingsPageView>) -> Self {
        ...
        let page = PageType::new_uncategorized(
            widgets,
            Some(tr(current_locale(), MessageId::SettingsAccount).into_owned()),
        );
...
impl AccountWidget {
    fn render_anonymous_account_info(
        ...
        let compare_plans = tr(current_locale(), MessageId::SettingsComparePlans);
        plan_info.add_child(
            Container::new(
                appearance
                    .ui_builder()
        ...
        if let Some(team) = workspaces.current_team() {
                ...
                if team.billing_metadata.customer_type == CustomerType::Enterprise {
                    plan_info.add_child(
                        appearance
                            .ui_builder()
                            .link(
                                tr(current_locale(), MessageId::SettingsContactSupport)
                                    .into_owned(),
                    ...
                    if team.has_billing_history {
                        ...
                        plan_info.add_child(
                            appearance
                                .ui_builder()
                                .link(
                                    tr(current_locale(), MessageId::SettingsManageBilling)
                                        .into_owned(),
                                    None,
                    ...
                    if team.billing_metadata.can_upgrade_to_higher_tier_plan() {
                        let description = match team.billing_metadata.customer_type {
                            CustomerType::Prosumer => "Upgrade to Turbo plan".to_owned(),
                            CustomerType::Turbo => "Upgrade to Lightspeed plan".to_owned(),
                            _ => tr(current_locale(), MessageId::SettingsComparePlans).into_owned(),
                        };
                        let team_uid = team.uid;
                        plan_info.add_child(
                            appearance
            ...
            plan_info.add_child(
                appearance
                    .ui_builder()
                    .link(
                        tr(current_locale(), MessageId::SettingsComparePlans).into_owned(),
                        None,
                        Some(Box::new(move |ctx| {
                            ctx.dispatch_typed_action(MainPageAction::Upgrade {
                                team_uid: None,
...
impl SettingsWidget for SettingsSyncWidget {
    ...
    fn render(
        ...
        Container::new(render_body_item::<MainPageAction>(
            tr(current_locale(), MessageId::SettingsSettingsSync).into_owned(),
            Some(label_info),
        ...
        Container::new(
            self.render_row(
                ...
                appearance
                    .ui_builder()
                    .link(
                        tr(current_locale(), MessageId::AppReferAFriend).into_owned(),
                        None,
                        Some(Box::new(move |ctx| {
            ...
            .is_enabled()
        {
            ...
            match autoupdate::get_update_state(app) {
                AutoupdateStage::NoUpdateAvailable => (
                    Some(StatusContent {
                        text: tr(current_locale(), MessageId::SettingsUpToDate).into_owned(),
                        color: faded_text_color,
                    }),
                    Some(CallToActionContent {
                        text: tr(current_locale(), MessageId::SettingsCheckForUpdates).into_owned(),
                        action: MainPageAction::CheckForUpdate,
                    }),
                ),
                AutoupdateStage::CheckingForUpdate => (
                    Some(StatusContent {
                        text: "checking for update...".to_owned(),
                        color: faded_text_color,
                    }),
                    None,
                ),
                AutoupdateStage::DownloadingUpdate => (
                    Some(StatusContent {
                        text: "downloading update...".to_owned(),
                        color: faded_text_color,
                    }),
                    None,
                ),
                AutoupdateStage::UpdateReady { .. } => (
                    Some(StatusContent {
                        text: tr(current_locale(), MessageId::SettingsUpdateAvailable).into_owned(),
                        color: ansi_red,
                    }),
                    Some(CallToActionContent {
                        text: tr(current_locale(), MessageId::SettingsRelaunchWarp).into_owned(),
                        action: MainPageAction::Relaunch,
                    }),
                ),
                AutoupdateStage::Updating { .. } => (
                    Some(StatusContent {
                        text: tr(current_locale(), MessageId::SettingsUpdating).into_owned(),
                        color: faded_text_color,
                    }),
                    None,
                ),
                AutoupdateStage::UpdatedPendingRestart { .. } => (
                    Some(StatusContent {
                        text: tr(current_locale(), MessageId::SettingsInstalledUpdate).into_owned(),
                        color: faded_text_color,
                    }),
                    Some(CallToActionContent {
                        text: tr(current_locale(), MessageId::SettingsRelaunchWarp).into_owned(),
                ...
                AutoupdateStage::UnableToUpdateToNewVersion { .. } => (
                    ...
                    Some(CallToActionContent {
                        text: tr(current_locale(), MessageId::SettingsUpdateManually).into_owned(),
        ...
        let mut first_row = Flex::row()
            ...
            .with_child(
                Shrinkable::new(
                    ...
                    Align::new(
                        Text::new_inline(
                            tr(current_locale(), MessageId::SettingsVersion).into_owned(),
                            appearance.ui_font_family(),
                            REGULAR_TEXT_FONT_SIZE,
                        )
