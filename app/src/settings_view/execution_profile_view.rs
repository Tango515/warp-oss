...
use crate::settings_view::settings_page::settings_display_text;
...
impl ExecutionProfileView {
    pub fn new(profile_id: ClientProfileId, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let edit_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(settings_display_text("Edit"), SecondaryTheme)
...
impl View for ExecutionProfileView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let cli_agent_model = profile
            ...
            .and_then(|id| llm_preferences.get_llm_info(id))
            .map(|info| info.display_name.clone())
            .unwrap_or_else(|| settings_display_text("Auto").to_string());
                ...
                .with_child({
                    ...
                    model_flex.add_child(with_standard_vertical_margin(
                        render_model_line_with_icon(
                            Icon::Lightning,
                            settings_display_text("Base model:"),
                            base_model,
                            appearance,
                            is_any_ai_enabled,
                        ),
                    ...
                    model_flex.add_child(with_standard_vertical_margin(
                        render_model_line_with_icon(
                            Icon::Terminal,
                            settings_display_text("Full terminal use:"),
                            cli_agent_model,
                            appearance,
                            is_any_ai_enabled,
                        ),
                    ...
                    if FeatureFlag::LocalComputerUse.is_enabled() {
                        model_flex.add_child(with_standard_vertical_margin(
                            render_model_line_with_icon(
                                Icon::Laptop,
                                settings_display_text("Computer use:"),
                                computer_use_model,
                                appearance,
                                is_any_ai_enabled,
                            ),
                    ...
                    Container::new({
                        let mut permissions_column = Flex::column()
                            .with_child(
                                Container::new(
                                    Text::new(
                                        settings_display_text("PERMISSIONS"),
                                        appearance.ui_font_family(),
                                        10.,
                                    )
                                    .with_color(appearance.theme().disabled_ui_text_color().into())
                            ...
                            .with_child(with_standard_vertical_margin(
                                render_action_permission_line_with_icon(
                                    Icon::Code2,
                                    settings_display_text("Apply code diffs:"),
                            ...
                            .with_child(with_standard_vertical_margin(
                                render_action_permission_line_with_icon(
                                    Icon::Notebook,
                                    settings_display_text("Read files:"),
                                    &profile.read_files,
                                    appearance,
                                    is_any_ai_enabled,
                                ),
                        ...
                        permissions_column.add_child(with_standard_vertical_margin(
                            render_action_permission_line_with_icon(
                                Icon::Terminal,
                                settings_display_text("Execute commands:"),
                        ...
                        permissions_column.add_child(with_standard_vertical_margin(
                            render_write_to_pty_permission_line_with_icon(
                                Icon::Workflow,
                                settings_display_text("Interact with running commands:"),
                        ...
                        permissions_column.add_child(with_standard_vertical_margin(
                            render_ask_user_question_permission_line_with_icon(
                                Icon::MessageText,
                                settings_display_text("Ask questions:"),
                        ...
                        permissions_column.add_child(with_standard_vertical_margin(
                            render_action_permission_line_with_icon(
                                Icon::Dataflow,
                                settings_display_text("Call MCP servers:"),
                        ...
                        if FeatureFlag::WebSearchUI.is_enabled() {
                            permissions_column.add_child(with_standard_vertical_margin(
                                render_bool_permission_line_with_icon(
                                    Icon::Globe,
                                    settings_display_text("Call web tools:"),
                        ...
                        permissions_column.add_child(with_standard_vertical_margin(
                            render_bool_permission_line_with_icon(
                                Icon::Compass,
                                settings_display_text("Auto-sync plans to Warp Drive:"),
...
fn render_action_permission_line_with_icon(
    ...
    let permission_text = match permission {
        ActionPermission::AgentDecides => settings_display_text("Agent decides"),
        ActionPermission::AlwaysAllow => settings_display_text("Always allow"),
        ActionPermission::AlwaysAsk => settings_display_text("Always ask"),
        ActionPermission::Unknown => settings_display_text("Unknown"),
    };
...
fn render_write_to_pty_permission_line_with_icon(
    ...
    let permission_text = match permission {
        WriteToPtyPermission::AlwaysAllow => settings_display_text("Always allow"),
        WriteToPtyPermission::AlwaysAsk => settings_display_text("Always ask"),
        WriteToPtyPermission::AskOnFirstWrite => settings_display_text("Ask on first write"),
        WriteToPtyPermission::Unknown => settings_display_text("Unknown"),
    };
    render_permission_line_with_icon(icon, label, permission_text, appearance, is_ai_enabled)
}
...
fn render_ask_user_question_permission_line_with_icon(
    ...
    let permission_text = match permission {
        AskUserQuestionPermission::Never => settings_display_text("Never ask"),
        AskUserQuestionPermission::AskExceptInAutoApprove | AskUserQuestionPermission::Unknown => {
            settings_display_text("Ask unless auto-approve")
        }
        AskUserQuestionPermission::AlwaysAsk => settings_display_text("Always ask"),
    };
    render_permission_line_with_icon(icon, label, permission_text, appearance, is_ai_enabled)
}
...
fn render_bool_permission_line_with_icon(
    ...
    let permission_text = if enabled {
        settings_display_text("On")
    } else {
        settings_display_text("Off")
    };
    render_permission_line_with_icon(icon, label, permission_text, appearance, is_ai_enabled)
}
...
fn render_directory_allowlist(
    ...
    with_standard_vertical_margin(render_pathbuf_allowlist_row(
        Icon::Check,
        settings_display_text("Directory allowlist:").to_string(),
...
fn render_command_allowlist(
    ...
    with_standard_vertical_margin(render_command_predicate_row(
        Icon::Check,
        settings_display_text("Command allowlist:").to_string(),
...
fn render_command_denylist(
    ...
    with_standard_vertical_margin(render_command_predicate_row(
        Icon::SlashCircle,
        settings_display_text("Command denylist:").to_string(),
...
fn render_mcp_allowlist(
    ...
    with_standard_vertical_margin(render_mcp_uuid_row(
        Icon::Check,
        settings_display_text("MCP allowlist:").to_string(),
...
fn render_mcp_denylist(
    ...
    with_standard_vertical_margin(render_mcp_uuid_row(
        Icon::SlashCircle,
        settings_display_text("MCP denylist:").to_string(),
