...
use super::{
    ...
    settings_page::{
        ...
        settings_display_text, AdditionalInfo, Category, PageType, SettingsPageMeta,
...
pub fn create_discount_badge(discount: u32, appearance: &Appearance) -> Box<dyn Element> {
    ...
    Container::new(
        Text::new_inline(
            settings_display_text(format!("{discount}% off")),
...
impl BillingUsageTab {
    pub fn get_tab_from_label(label: &str) -> Self {
        if label == &*tr(current_locale(), MessageId::BillingOverview) {
            BillingUsageTab::Overview
        } else if label == &*tr(current_locale(), MessageId::BillingUsageHistory) {
    ...
    pub fn label(&self) -> std::borrow::Cow<'static, str> {
        match self {
            BillingUsageTab::Overview => tr(current_locale(), MessageId::BillingOverview),
            BillingUsageTab::UsageHistory => tr(current_locale(), MessageId::BillingUsageHistory),
        }
    }
}
...
impl BillingAndUsagePageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let overage_limit_modal_view = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(settings_display_text("Overage spending limit")),
                overage_limit_modal,
                ctx,
            )
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            BillingAndUsagePageAction::ToggleSortingMenu => {
                ...
                let sort_options = [
                    (
                        tr(current_locale(), MessageId::BillingAtoZ),
                        SortKey::DisplayName,
                        SortOrder::Asc,
                    ),
                    (
                        tr(current_locale(), MessageId::BillingZtoA),
                        SortKey::DisplayName,
                        SortOrder::Desc,
                    ),
                    (
                        tr(current_locale(), MessageId::BillingUsageAscending),
                        SortKey::Requests,
                        SortOrder::Asc,
                    ),
                    (
                        tr(current_locale(), MessageId::BillingUsageDescending),
                        SortKey::Requests,
                        SortOrder::Desc,
                    ),
                ];
...
impl UsageWidget {
    ...
    fn render_ambient_agent_trial_widget(
        ...
        let title = Text::new_inline(
            settings_display_text(AMBIENT_AGENT_TRIAL_TITLE),
            appearance.ui_font_family(),
            14.,
        )
        ...
        let credits_label = Text::new_inline(
            settings_display_text(credits_text),
            appearance.ui_font_family(),
            12.,
        )
        ...
        if credits_remaining >= AMBIENT_AGENT_TRIAL_CREDIT_THRESHOLD {
            let new_agent_button = ui_builder
                ...
                .with_text_label(settings_display_text("New agent"))
        ...
        if !is_on_paid_plan {
            ...
            let buy_more_button = ui_builder
                ...
                .with_text_label(settings_display_text("Buy more"))
                .with_style(UiComponentStyles {
    ...
    fn render_usage_based_pricing_section(
        ...
        let (header_text, description_text): (std::borrow::Cow<'static, str>, &str) =
            if has_admin_permissions {
                (
                    tr(current_locale(), MessageId::BillingEnableOverages),
                    OVERAGE_TOGGLE_DESCRIPTION,
                )
            } else if enabled {
                (
                    tr(current_locale(), MessageId::BillingOveragesEnabled),
                    OVERAGE_TOGGLE_DESCRIPTION,
                )
            } else {
                (
                    tr(current_locale(), MessageId::BillingOveragesNotEnabled),
    ...
    fn render_monthly_overage_spending_limit(
        ...
        let info_icon = render_info_icon(
            ...
            AdditionalInfo::<BillingAndUsagePageAction> {
                ...
                secondary_text: None,
                tooltip_override_text: Some(settings_display_text(
        ...
        let label = Text::new_inline(
            settings_display_text("Monthly overage spending limit"),
            appearance.ui_font_family(),
            12.,
        )
        .with_color(appearance.theme().active_ui_text_color().into())
        ...
        if has_admin_permissions {
            Some(
                appearance
                    .ui_builder()
                    .link(
                        tr(current_locale(), MessageId::BillingViewOverageDetails).into_owned(),
                        None,
    ...
    fn render_addon_credits_panel(
        ...
        let header = Text::new_inline(
            settings_display_text("Add-on credits"),
            appearance.ui_font_family(),
            16.,
        )
        .with_color(fg.into())
        ...
        let no_credits_access_explanation = match (
            ...
            (false, true, true) => {
                ...
                let (link_text, suffix) = if is_legacy_paid {
                    (
                        settings_display_text("Switch to the Build plan"),
                        settings_display_text(" to purchase add-on credits."),
                    )
                } else {
                    (
                        settings_display_text("Upgrade to the Build plan"),
                        settings_display_text(" to purchase add-on credits."),
                    )
                };
            ...
            // Account Executive.
            (false, false, true) => {
                let paragraph_text = settings_display_text(
            ...
            (_, _, false) => {
                let paragraph_text =
                    settings_display_text("Contact a team admin to purchase add-on credits.");
                Some(
                    ui_builder
                        .paragraph(paragraph_text)
                        .with_style(UiComponentStyles {
        ...
        let paragraph = ui_builder
            .paragraph(settings_display_text(paragraph_text))
            .with_style(UiComponentStyles {
                font_color: Some(theme.sub_text_color(bg).into()),
                ..Default::default()
            })
        ...
        let monthly_spend_row = Flex::row()
            ...
            .with_children([
                ui_builder
                    .span(settings_display_text("Monthly spend limit"))
                    .build()
                    .finish(),
        ...
        let purchased_this_month_row = if let Some(bonus_grants) = bonus_grants_purchased {
            if bonus_grants.total_credits_purchased == 0 {
                ...
                let label = Text::new_inline(
                    settings_display_text("Purchased this month"),
                    appearance.ui_font_family(),
                    12.,
                )
                .with_color(appearance.theme().active_ui_text_color().into())
        ...
        let auto_reload_switch = Container::new(render_body_item::<BillingAndUsagePageAction>(
            settings_display_text("Auto reload"),
            None,
            Default::default(),
            Default::default(),
            appearance,
            auto_reload_switch,
            Some(settings_display_text(format!(
        ...
        if auto_reload_enabled {
            ...
            if delinquent_due_to_payment_issue {
                card_content_upper.add_child(self.render_warning_row(
                    appearance,
                    settings_display_text(AUTO_RELOAD_DELINQUENT_WARNING_STRING),
                ));
            } else if would_exceed_limit {
                card_content_upper.add_child(self.render_warning_row(
                    appearance,
                    settings_display_text(AUTO_RELOAD_EXCEED_LIMIT_WARNING_STRING),
                ));
            }
            ...
            let mut card_content_lower_children = vec![
                ui_builder
                    .span(settings_display_text("One-time purchase"))
                    .build()
                    .finish(),
                buy_row.finish(),
            ];
                ...
                .has_failed_addon_credit_auto_reload_status()
            {
                card_content_lower_children.push(self.render_warning_row(
                    appearance,
                    settings_display_text(RESTRICTED_BILLING_USAGE_WARNING_STRING),
                ));
            } else if would_exceed_limit {
                let warning_fragments = vec![
                    FormattedTextFragment::plain_text(settings_display_text(
                        "Reloading would exceed your monthly limit. ",
                    )),
                    FormattedTextFragment::hyperlink_action(
                        settings_display_text("Increase your limit"),
                        BillingAndUsagePageAction::ShowAddOnCreditModal,
                    ),
                    FormattedTextFragment::plain_text(settings_display_text(" to continue.")),
                ];
    ...
    fn render_total_overages_row(
        ...
        let label = Text::new_inline(
            settings_display_text("Total overages"),
            appearance.ui_font_family(),
            12.,
        )
        .with_color(appearance.theme().active_ui_text_color().into())
...
impl SettingsWidget for UsageWidget {
    ...
    fn render(
        ...
        let tab_selector = tab_selector::render_tab_selector(
            tabs,
            view.selected_tab.label().as_ref(),
...
impl UsageWidget {
    fn render_usage_history_content(
        ...
        let mut content = Flex::column()
            ...
            .with_child(
                Container::new(
                    Text::new_inline(
                        settings_display_text("Last 30 days"),
                        appearance.ui_font_family(),
                        14.,
                    )
                    .with_color(blended_colors::text_sub(
        ...
        if is_loading {
            ...
            let res = Flex::column()
                ...
                .with_child(
                    Container::new(
                        Text::new(
                            settings_display_text("No usage history"),
                            appearance.ui_font_family(),
                            14.,
                        )
                        .with_color(blended_colors::text_sub(
    ...
    pub fn render_enterprise_usage_card(
        ...
        let header = Text::new_inline(
            settings_display_text(ENTERPRISE_USAGE_CALLOUT_HEADER),
            appearance.ui_font_family(),
            16.,
        )
        ...
        let body = if has_admin_permissions {
            ...
            let text_fragments = vec![
                ...
                FormattedTextFragment::hyperlink(
                    settings_display_text(ENTERPRISE_USAGE_CALLOUT_BODY_ADMIN_LINK),
                    admin_panel_url,
                ),
                FormattedTextFragment::plain_text(settings_display_text(
    ...
    fn render_usage_content(
        ...
        let mut usage_header_right_side = Flex::row()
            ...
            .with_child(
                appearance
                    .ui_builder()
                    .paragraph(settings_display_text(format!(
            ...
            UserWorkspaces::as_ref(app).current_team()
        {
            if workspace_is_delinquent_due_to_payment_issue {
                if has_admin_permissions {
                    ...
                    vec![FormattedTextFragment::plain_text(tr(
                        current_locale(),
                        MessageId::BillingEnterpriseUsageCallout,
                    ))]
                }
                ...
                vec![
                    ...
                    FormattedTextFragment::plain_text(settings_display_text(" for more AI usage.")),
                ]
            } else {
                vec![]
            }
...
impl PlanWidget {
    fn render_anonymous_account_info(
        ...
        let user_info = appearance
            ...
            .with_text_label(tr(current_locale(), MessageId::SettingsSignUp).into_owned())
            .build()
            .on_click(move |ctx, _, _| {
        ...
        plan_info.add_child(render_customer_type_badge(
            appearance,
            tr(current_locale(), MessageId::SettingsFree).into_owned(),
        ));
        plan_info.add_child(
            Container::new(
                appearance
                    ...
                    .with_text_and_icon_label(
                        TextAndIcon::new(
                            ...
                            tr(current_locale(), MessageId::SettingsComparePlans),
    ...
    fn render_non_team_user_actions(
        ...
        let plan_badge = render_customer_type_badge(
            appearance,
            tr(current_locale(), MessageId::SettingsFree).into_owned(),
        );
