...
use crate::{
    ...
    settings_view::settings_page::settings_display_text,
...
impl MCPServersEditPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let save_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(
                tr(current_locale(), MessageId::KeybindingsSave).into_owned(),
                PrimaryTheme,
            )
        ...
        let reinstall_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(settings_display_text("Edit Variables"), PrimaryTheme).on_click(
                |ctx| {
        ...
        let delete_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(settings_display_text("Delete MCP"), DangerSecondaryTheme)
        ...
        let unshare_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(settings_display_text("Remove from team"), DangerNakedTheme)
        ...
        let editing_disabled_banner = ctx.add_typed_action_view(|_| {
            Banner::new_without_close(BannerTextContent::plain_text(settings_display_text(
    ...
    fn render_header(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let title = if self.server_card_item_id.is_none() {
            settings_display_text("Add New MCP Server").to_string()
        } else if let Some(name) = self.server_model.name() {
            format!(
                "{} {name} {}",
                settings_display_text("Edit"),
                settings_display_text("MCP Server")
            )
        } else {
            settings_display_text("Edit MCP Server").to_string()
        };
        ...
        .with_tooltip(move || {
            ui_builder
                .tool_tip(settings_display_text("Log out").to_string())
                .build()
                .finish()
        })
        .build()
        ...
        Container::new(
            Flex::column()
                ...
                .with_child(
                    Container::new(
                        Container::new(
                            Text::new(settings_display_text("JSON"), ui_font_family, font_size)
                                .finish(),
                        )
        ...
        if contains_secrets {
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                toast_stack.add_ephemeral_toast(
                    DismissibleToast::error(settings_display_text("This MCP server contains secrets. Visit Settings > Privacy to modify your secret redaction settings.").to_string()),
                    window_id,
                    ctx,
                );
            });
            return Err(settings_display_text("This MCP server contains secrets. Visit Settings > Privacy to modify your secret redaction settings.").to_string());
        }
    ...
    fn build_templatable_mcp_server_result_from_json(
        ...
        if parsed_templatable_mcp_servers.is_empty() {
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                toast_stack.add_ephemeral_toast(
                    DismissibleToast::error(
                        settings_display_text("No MCP Server specified.").to_string(),
                    ),
                    window_id,
                    ctx,
                );
            ...
            return Err(settings_display_text("No MCP Server specified.").to_string());
        }
        ...
        if parsed_templatable_mcp_servers.len() > 1 {
            ...
            return Err(settings_display_text(
                "Cannot add multiple MCP servers while editing a single server.",
            )
            .to_string());
        }
