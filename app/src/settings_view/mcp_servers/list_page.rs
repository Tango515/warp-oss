...
use crate::settings_view::settings_page::{
    build_toggle_element, render_body_item_label, settings_display_text, LocalOnlyIconState,
    ToggleState,
};
...
impl MCPServersListPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        search_editor.update(ctx, |editor, ctx| {
            ...
            editor.set_placeholder_text(&settings_display_text("Search MCP Servers"), ctx);
        });
        let search_bar = ctx.add_typed_action_view(|_| SearchBar::new(search_editor.clone()));

        let add_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(settings_display_text("Add"), NakedTheme)
                .with_icon(Icon::Plus)
    ...
    fn create_template_server_card(
        ...
        let server_card = ServerCardView::new(
            ...
            template
                .description
                .clone()
                .or_else(|| Some(settings_display_text("Available to install").to_string())),
            ...
            MCPServerUpdate::CloudTemplate { .. } => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::success(
                        settings_display_text("MCP server updated").to_string(),
                    );
    ...
    fn render_file_based_mcp_section(
        ...
        let label = render_body_item_label::<MCPServersListPageViewAction>(
            settings_display_text("Auto-spawn servers from third-party agents").to_string(),
            None,
            None,
        ...
        let file_based_description_fragments = vec![
            ...
            FormattedTextFragment::hyperlink(
                settings_display_text("See supported providers."),
    ...
    fn render_page_body(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let description_fragments = vec![
            FormattedTextFragment::plain_text(settings_display_text(DESCRIPTION_TEXT)),
            FormattedTextFragment::hyperlink(
                settings_display_text("Learn more."),
                ...
                && filtered_file_based_cards.is_empty()
            {
                ...
                if !shared_server_cards.is_empty() {
                    ...
                    let shared_by_text = match team_name {
                        Some(name) => {
                            format!("{} {name}", settings_display_text("Shared by Warp and"))
                        }
                        None => settings_display_text("Shared by Warp and from other devices")
                            .to_string(),
                    };
                    ...
                    page.add_child(self.render_server_cards_section(
                        settings_display_text("Shared from Warp"),
                ...
                for (provider, cards) in &filtered_file_based_cards {
                    let section_title = format!(
                        "{} {}",
                        settings_display_text("Detected from"),
                ...
                Align::new(
                    Flex::column()
                        ...
                        .with_child(
                            appearance
                                .ui_builder()
                                .wrappable_text(settings_display_text(EMPTY_STATE_TEXT), true)
                ...
                Align::new(
                    Flex::column()
                        ...
                        .with_child(
                            appearance
                                .ui_builder()
                                .wrappable_text(settings_display_text(NO_SEARCH_RESULTS_TEXT), true)
                                .with_style(style::description_text(appearance))
                                .build()
                                .finish(),
                        )
    ...
    fn file_based_root_chip_text(root_path: &PathBuf) -> Option<String> {
        ...
        if let Some(home_dir) = dirs::home_dir() {
            if root_path == &home_dir {
                return Some(settings_display_text("global").to_string());
            }
        }
        ...
        if root_path == &crate::warp_managed_paths_watcher::warp_data_dir() {
            return Some(settings_display_text("global"));
        }
    ...
    fn get_file_based_title_chips(
        ...
        // chips are redundant).
        let global_text = settings_display_text("global").to_string();
        ...
        for provider in MCPProvider::iter() {
            ...
            let server_card = ServerCardView::new(
                ...
                installation
                    ...
                    .clone()
                    .or_else(|| {
                        Some(settings_display_text("Detected from config file").to_string())
                    }),
    ...
    fn get_title_chip_text(
        ...
        match item_id {
            ...
            | ServerCardItemId::TemplatableMCPInstallation(_) => {
                ...
                if is_shared {
                    match creator {
                        Some(creator) => Some(TitleChip::text(format!(
                            "{}: {creator}",
                            settings_display_text("Shared by")
                        ))),
                        None => Some(TitleChip::text(settings_display_text(
                            "Shared by a team member",
                        ))),
                    }
                } else if matches!(item_id, ServerCardItemId::TemplatableMCP(_)) {
                    Some(TitleChip::text(settings_display_text(
                        "From another device",
                    )))
                } else {
                    None
