...
impl UpdateModalBody {
    ...
    fn render_title(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let name = self
            .server_name
            .as_deref()
            .unwrap_or_else(|| Box::leak(settings_display_text("Server").into_boxed_str()));
        ...
        let title = Text::new(
            format!("{} {name}", settings_display_text("Update")),
    ...
    fn render_description(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let description = format!(
            "{} {} {}",
            settings_display_text("This server has"),
            self.update_options.len(),
            settings_display_text("updates available, which would you like to proceed with?")
        );
        ...
        let (title, description) = match option {
            MCPServerUpdate::CloudTemplate {
                ...
                let publisher_string = match publisher {
                    Author::CurrentUser => settings_display_text("another device").to_string(),
                    Author::OtherUser { name } => name.clone(),
                    Author::Unknown => settings_display_text("a team member").to_string(),
                };
                let datetime = Local
                ...
                (
                    format!(
                        "{} {publisher_string}",
                        settings_display_text("Update from")
                    ),
            ...
            MCPServerUpdate::Gallery {
                ...
                format!("{} {name}", settings_display_text("Update from")),
                format!("{} {new_version}", settings_display_text("Version")),
            ),
        };
    ...
    fn render_action_buttons(&self, appearance: &Appearance) -> Box<dyn Element> {
        let cancel_button = appearance
            ...
            .with_text_label(tr(current_locale(), MessageId::KeybindingsCancel).into_owned())
    ...
    fn render(&self, ctx: &AppContext) -> Box<dyn Element> {
        ...
        if self.update_options.is_empty() {
            let no_updates_text = Text::new(
                settings_display_text("No updates available"),
                appearance.ui_font_family(),
                appearance.ui_font_size(),
            )
            .finish();
