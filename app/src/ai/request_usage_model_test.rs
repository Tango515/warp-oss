...
#[test]
fn test_has_any_ai_remaining_true_with_local_byo_key_without_subscription_policy() {
    App::test((), |mut app| async move {
        let (_uid, workspace) = create_test_workspace();
        ...
        request_usage_model.update(&mut app, |model, ctx| {
            ...
            assert!(
                model.has_any_ai_remaining(ctx),
                "expected has_any_ai_remaining to be true when a local BYO key is provided",
            );
        });
    });
}
