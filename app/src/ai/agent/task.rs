...
macro_rules! debug_log {
    ($($arg:tt)*) => {
        ...
        {
            use std::io::Write;
            let _ = writeln!(f, "[{}] [Task] {}", chrono::Local::now().format("%H:%M:%S%.3f"), format!($($arg)*));
        }
    };
}
...
impl Task {
    ...
    pub fn is_server(&self) -> bool {
        matches!(self.data, TaskImpl::Server(_))
    }
    ...
    pub(super) fn append_to_message_content(
        ...
        let source_messages_count = self.try_get_source()?.messages.len();
        let Some((idx, existing_message)) = self
            .try_get_source()?
            .messages
            .iter()
            ...
            debug_log!("append_to_message_content: MessageNotFound! message_id={}, source_messages_count={}", message.id, source_messages_count);
            log::error!("Message not found for append client action.");
            return Err(UpdateTaskError::MessageNotFound);
        };
        debug_log!(
            "append_to_message_content: found message at idx={}, message_id={}",
            idx,
            message.id
        );
        let updated_message =
    ...
    fn upsert_output_for_message(
        ...
        match &self.output_status {
            AIAgentOutputStatus::Streaming {
                ...
                debug_log!("upsert_output_for_message: message_id={}, found_existing={}, output_messages_count={}", task_message.id, message_idx.is_some(), output.messages.len());
                match converted {
                    MaybeAIAgentOutputMessage::Message(m) => {
                        debug_log!(
                            "upsert_output_for_message: converted to Message, kind={:?}",
                        ...
                        debug_log!(
                            "upsert_output_for_message: output_messages_count after={}",
                            output.messages.len()
                        );
                    }
                    MaybeAIAgentOutputMessage::NoClientRepresentation => {
                        debug_log!("upsert_output_for_message: NoClientRepresentation!");
                        log::warn!(
            ...
            AIAgentOutputStatus::Streaming { output: None } => {
                debug_log!("upsert_output_for_message: output is None (never initialized)!");
            }
            _ => {
                debug_log!("upsert_output_for_message: output_status is not Streaming!");
            }
        }
