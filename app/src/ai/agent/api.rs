...
macro_rules! debug_log {
    ($($arg:tt)*) => {
        ...
        {
            use std::io::Write;
            let _ = writeln!(f, "[{}] [API] {}", chrono::Local::now().format("%H:%M:%S%.3f"), format!($($arg)*));
        }
    };
}
...
pub struct RequestParams {
    ...
    /// Configuration for the local AI agent provider. When `Some`, requests are routed
    /// to this endpoint instead of the Warp server.
    pub local_provider_config: Option<local_provider::LocalProviderConfig>,
    /// The root task ID of the conversation, used by the local provider to match events.
...
impl RequestParams {
    pub fn new(
        ...
        // Route to the local provider whenever all required fields are configured.
        let base_url = ai_settings.local_agent_provider_base_url.value().clone();
        let model = ai_settings.local_agent_provider_model.value().clone();
        let api_key = ai_settings.local_agent_provider_api_key.value().clone();
        debug_log!(
            "LOCAL PROVIDER: base_url={}, model={}, api_key_len={}",
            base_url,
            model,
            api_key.len()
        );
        let local_provider_config =
            if !base_url.is_empty() && !model.is_empty() && !api_key.is_empty() {
                debug_log!("LOCAL PROVIDER: Config complete, routing to local endpoint");
                Some(local_provider::LocalProviderConfig {
                    base_url,
                    model,
                    api_key,
                })
                ...
                debug_log!(
                    "LOCAL PROVIDER: Config incomplete (empty fields), falling back to Warp server"
                );
                None
            };
