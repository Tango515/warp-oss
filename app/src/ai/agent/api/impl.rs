...
macro_rules! debug_log {
    ($($arg:tt)*) => {
        ...
        {
            use std::io::Write;
            let _ = writeln!(f, "[{}] [Impl] {}", chrono::Local::now().format("%H:%M:%S%.3f"), format!($($arg)*));
        }
    };
}
...
pub async fn generate_multi_agent_output(
    ...
    if let Some(config) = params.local_provider_config.take() {
        debug_log!("Routing to local provider: {}", config.base_url);
        return local_provider::generate_local_agent_output(config, params, cancellation_rx)
            .await
            .map_err(|e| ConvertToAPITypeError::Other(anyhow::anyhow!("{}", e)));
    }
    debug_log!("Using default Warp server (no local config)");
