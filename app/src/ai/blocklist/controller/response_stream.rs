...
macro_rules! debug_log {
    ($($arg:tt)*) => {
        ...
        {
            use std::io::Write;
            let _ = writeln!(f, "[{}] [ResponseStream] {}", chrono::Local::now().format("%H:%M:%S%.3f"), format!($($arg)*));
        }
    };
}
...
impl ResponseStream {
    ...
    fn handle_response_stream_result(
        ...
        match stream_result {
            Ok(stream) => {
                debug_log!(
                    "[ResponseStream] Stream obtained successfully, spawning stream listener"
                );
            ...
            Err(e) => {
                debug_log!("[ResponseStream] ERROR: Failed to get stream: {e:?}");
    ...
    fn on_response_stream_complete(&mut self, request_id: Uuid, ctx: &mut ModelContext<Self>) {
        if self.current_request_id.is_none_or(|id| id != request_id) {
            debug_log!("[ResponseStream] on_complete: request_id mismatch, skipping");
            return;
        }
        debug_log!("[ResponseStream] on_complete: emitting AfterStreamFinished");
