...
macro_rules! debug_log {
    ($($arg:tt)*) => {
        ...
        {
            use std::io::Write;
            let _ = writeln!(f, "[{}] [Controller] {}", chrono::Local::now().format("%H:%M:%S%.3f"), format!($($arg)*));
        }
    };
}
...
impl BlocklistAIController {
                ...
                match event {
                    Ok(event) => {
                        ...
                        let Some(event) = event.r#type else {
                            debug_log!("WARNING: event.r#type is None");
                            return;
                        };
                        debug_log!("Received event type: {:?}", std::mem::discriminant(&event));
                        match event {
                            warp_multi_agent_api::response_event::Type::Init(init_event) => {
                                debug_log!(
                                    "Processing Init event, conversation_id={}",
                            ...
                            warp_multi_agent_api::response_event::Type::Finished(
                                ...
                                debug_log!(
                                    "Processing Finished event, reason={:?}",
                            ...
                            warp_multi_agent_api::response_event::Type::ClientActions(actions) => {
                                ...
                                debug_log!(
                                    "Processing ClientActions, count={}",
                                    client_actions.len()
                                );
                                let apply_result =
                                    history_model.update(ctx, |history_model, ctx| {
                                ...
                                if let Err(e) = apply_result {
                                    debug_log!("ERROR: Failed to apply client actions: {e:?}");
                                    log::error!(
                                        "Failed to apply client actions to conversation: {e:?}"
                                    );
                                } else {
                                    debug_log!("ClientActions applied successfully");
                                }
                            }
                        }
                    }
    ...
    pub(super) fn handle_response_stream_finished(
        ...
        debug_log!("handle_response_stream_finished: stream_id={:?}", stream_id);
        let history_model = BlocklistAIHistoryModel::handle(ctx);
        history_model.update(ctx, |history_model, _| {
        ...
        match finished_event.reason {
            Some(warp_multi_agent_api::response_event::stream_finished::Reason::Done(_)) | None => {
                debug_log!("Stream finished with Done reason");
                history_model.update(ctx, |history_model, ctx| {
