...
pub fn is_free_user_no_ai_experiment_active(_ctx: &AppContext) -> bool {
    // OSS builds allow users to bring their own provider keys, so the free-tier
    // no-AI experiment should not lock the local Agent entrypoint.
    false
}
