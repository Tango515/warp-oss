...
pub fn natural_language_words_score(words: Vec<Cow<str>>, is_first_token_command: bool) -> usize {
    ...
    for (i, token) in words.into_iter().enumerate() {
        ...
        if is_cjk_natural_language_token(&token)
            || is_word(&token, WordDb::StackOverflow)
            || is_word(&token, WordDb::Command)
        {
...
pub fn is_cjk_natural_language_token(word: &str) -> bool {
    !wrapped_in_quotes(word)
        && !check_if_token_has_shell_syntax(word)
        && word.chars().any(is_cjk_character)
}
