...
pub struct ApiKeys {
    ...
    /// API key for the local AI agent provider (OpenAI-compatible endpoint).
...
impl ApiKeyManager {
    ...
    pub fn set_local_agent_key(&mut self, key: Option<String>, ctx: &mut ModelContext<Self>) {
        self.keys.local_agent = key;
        ctx.emit(ApiKeyManagerEvent::KeysUpdated);
        self.write_keys_to_secure_storage(ctx);
    }
