...
use natural_language_detection::{
    check_if_token_has_shell_syntax, is_cjk_natural_language_token, natural_language_words_score,
};
...
impl InputClassifier for HeuristicClassifier {
    async fn detect_input_type(&self, input: ParsedTokensSnapshot, context: &Context) -> InputType {
        ...
        let has_cjk_natural_language_token = has_cjk_natural_language_token(&word_tokens);
        let starts_with_explicit_shell_command = starts_with_explicit_shell_command(&input);
        if has_cjk_natural_language_token && !starts_with_explicit_shell_command {
            return InputType::AI;
        }

        if (!has_cjk_natural_language_token || starts_with_explicit_shell_command)
            && is_likely_shell_command(&input, total_word_token_count).await
        {
...
fn has_cjk_natural_language_token(word_tokens: &[String]) -> bool {
    word_tokens
        .iter()
        .any(|token| is_cjk_natural_language_token(token))
}

fn starts_with_explicit_shell_command(input: &ParsedTokensSnapshot) -> bool {
    input.parsed_tokens.first().is_some_and(|token| {
        token.token_description.is_some()
            ...
            || is_one_off_shell_command_keyword(token.token.as_str())
    })
}
...
async fn natural_language_detection_heuristic(
    ...
    let has_cjk_natural_language_token = word_tokens
        .iter()
        .any(|token| is_cjk_natural_language_token(token));

    if min_token_length > word_tokens_count as u8 && !has_cjk_natural_language_token {
