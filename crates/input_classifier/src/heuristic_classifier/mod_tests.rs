...
fn test_input_detection() {
    futures::executor::block_on(async move {
        ...
        // CJK prompts are natural language even when they are a single token.
        let mut token = mock_parsed_input_token("你好啊".to_string()).await;
        token.parsed_tokens[0].token_description = None;
        assert_eq!(
            classifier.detect_input_type(token.clone(), &context).await,
            InputType::AI
        ...
        // CJK prompts should recover from shell mode after a previous autodetection miss.
        let mut token = mock_parsed_input_token("你能直接操作终端么".to_string()).await;
        token.parsed_tokens[0].token_description = None;
        assert_eq!(
            classifier.detect_input_type(token, &context).await,
            InputType::AI,
        ...
        // CJK prompts often include spaces around command names. They should
        // stay in Agent mode instead of executing the full sentence as shell.
        let mut token = mock_parsed_input_token("请运行 pwd 看看当前目录".to_string()).await;
        token.parsed_tokens[0].token_description = None;
        assert_eq!(
            classifier.detect_input_type(token, &context).await,
            InputType::AI,
